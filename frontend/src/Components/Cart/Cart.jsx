        ...
        <div className="flex flex-col xl:flex-row gap-8 items-start">
            ...
            <div className="w-full xl:w-2/3">
                <form action="#" className="woocommerce-cart-form">
                    ...
                    <thead>
                        <tr>
                        <th className="cart-col-image bg-gray-100 p-4 text-left border-b">Image</th>
                        <th className="cart-col-productname bg-gray-100 p-4 text-left border-b">Product Name</th>
                        <th className="cart-col-price bg-gray-100 p-4 text-left border-b">Price</th>
                        <th className="cart-col-quantity bg-gray-100 p-4 text-left border-b">Quantity</th>
                        <th className="cart-col-total bg-gray-100 p-4 text-left border-b">Total</th>
                        <th className="cart-col-remove bg-gray-100 p-4 text-left border-b">Remove</th>
                        </tr>
                    </thead>
                    <tbody>
                        {cartItems.map((item) => (
                            <tr className="cart_item border-b border-gray-200" key={item.id}>
                                <td data-title="Product" className="p-4">
                                    <a className="cart-productimage block w-max" href="#">
                                        <img 
                                            src={item.m_image ? `${IMAGE_BASE_URL}/${item.m_image}` : "assets/img/product/placeholder.png"} 
                                ...
                                <td data-title="Name" className="p-4">
                                    <a className="cart-productname text-gray-800 font-bold hover:text-amber-500 transition" href="#">
                                        {item.m_menu_name}
                                    </a>
                                </td>
                                <td data-title="Price" className="p-4">
                                    <span className="amount font-bold text-amber-600">
                                        <bdi><span>Tk </span>{Number(item.m_price).toLocaleString()}</bdi>
                                    </span>
                                </td>
                                <td data-title="Quantity" className="p-4">
                                    <div className="quantity flex items-center gap-2">
                                        <button 
                                            type="button" 
                                            className="quantity-minus qty-btn bg-gray-200 p-2 rounded hover:bg-gray-300 transition"
                                ...
                                <td data-title="Total" className="p-4">
                                    <span className="amount font-bold text-gray-900">
                                        <bdi><span>Tk </span>{(Number(item.m_price) * item.quantity).toLocaleString()}</bdi>
                                    </span>
                                </td>
            ...
            <div className="w-full xl:w-1/3">
                <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <table className="cart_totals w-full mb-6">
                        <tbody>
                            <tr className="border-b border-gray-200">
                                <th className="py-3 text-left font-medium text-gray-600">Cart Subtotal</th>
                                <td className="py-3 text-right font-bold text-gray-800">
                                    <span className="amount"><bdi><span>Tk </span>{cartTotal.toLocaleString()}</bdi></span>
                                </td>
                            </tr>
                            
                            {/* Shipping Row - Fixed 60 Tk */}
                            <tr className="shipping border-b border-gray-200">
                                <th className="py-4 text-left font-medium text-gray-600 align-top">Shipping</th>
                                <td className="py-4 text-right">
                                    <div className="mb-2 font-bold text-gray-800">
                                        <span className="py-3 text-right font-bold text-gray-800">Tk {SHIPPING_COST}</span>
                                    </div>
                            ...
                            <tr className="order-total border-b border-gray-200">
                                <th className="py-4 text-left font-bold text-lg text-gray-800">Order Total</th>
                                <td className="py-4 text-right">
                                    <strong><span className="amount text-xl text-amber-600 font-bold"><bdi><span>Tk </span>{grandTotal.toLocaleString()}</bdi></span></strong>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div className="wc-proceed-to-checkout">
                        <a href="/checkout" className="th-btn bg-amber-500 text-white w-full block text-center py-3 rounded-md font-bold text-lg hover:bg-amber-600 transition shadow-md">
                            Proceed to checkout
                        </a>
                    </div>
                </div>
