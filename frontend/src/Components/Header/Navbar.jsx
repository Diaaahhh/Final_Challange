    ...
    <header className="relative w-full z-50 font-['Inter']">
          ...
          <div className="flex gap-6">
            <div className="flex items-center gap-2 hidden xl:flex">
              ...
              <span className="text-gray-300">
                House 33A, Road 4, Dhanmondi, Dhaka
              </span>
            </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex gap-3">
              {[
                {
                  Icon: FaFacebookF,
                  link: "https://www.facebook.com/IGLWebLtd/",
                },
      ...
      <div
        className={`transition-all duration-300 border-b border-white/5 ${
          isSticky
            ? "fixed top-0 left-0 w-full theme-bg shadow-lg py-2"
            : "relative theme-bg py-4"
        }`}
      >
        <div className="container mx-auto px-4">
          <div className="navbar p-0 min-h-[auto] flex justify-between items-center">
                ...
                <div className="w-[150px] h-[150px] flex items-center justify-center overflow-hidden">
                  {logo ? (
                    ...
                    <h2 className="text-3xl font-bold text-white">
                      <span className="theme-accent">Logo</span>
                    </h2>
                  )}
                </div>
              </Link>
              ...
              <ul className="menu menu-horizontal px-1 gap-6 font-bold text-white  tracking-wide text-[15px] font-['Barlow_Condensed']">
                {user && (user.role === 0 || user.role === "0") && (
                  <li>
                    <Link
                      to="/admin"
                      className="hover:text-white theme-accent p-0 bg-transparent"
                    >
                      Admin Panel
                    </Link>
                  </li>
                ...
                <li>
                  <Link
                    to="/"
                    className="hover-theme-accent focus:text-white p-0 bg-transparent"
                  >
                    Home
                  </Link>
                </li>
                <li>
                  <Link
                    to="/about"
                    className="hover-theme-accent p-0 bg-transparent"
                  >
                    About
                  </Link>
                </li>
                ...
                <li tabIndex={0} className="dropdown dropdown-hover group">
                  <span className="hover-theme-accent p-0 bg-transparent cursor-pointer">
                    Contact
                  </span>
                  <ul
                    tabIndex={0}
                    className="dropdown-content z-[1] menu p-2 shadow-lg bg-white text-black rounded-none w-52 text-sm mt-5 border-t-4 theme-border"
                  >
                    {/* <li><Link to="/address" className="hover-theme-accent hover:bg-transparent">Address</Link></li> */}
                    <li>
                      <Link
                        to="/branch_user"
                        className="hover-theme-accent hover:bg-transparent"
                      >
                        Branches
                      </Link>
                    </li>
            ...
            <div className="navbar-end flex gap-4 items-center w-full lg:w-auto">
              ...
              {user && (
                <div className="dropdown dropdown-end dropdown-hover group hidden xl:block mr-2">
                  <div
                    tabIndex={0}
                    role="button"
                    className="btn btn-ghost btn-circle avatar ring-2 theme-ring ring-offset-2 theme-ring-offset"
                  >
                  ...
                  <ul
                    tabIndex={0}
                    className="dropdown-content z-[1] menu p-2 shadow-lg bg-white text-black rounded-none w-40 text-sm mt-2 border-t-4 theme-border font-['Barlow_Condensed']  font-bold before:absolute before:-top-4 before:left-0 before:h-4 before:w-full before:content-['']"
                  >
                    <li>
                      <Link
                        to="/profile"
                    ...
                    <li className="border-t border-gray-200 mt-1 pt-1">
                      <a
                        ...
                        className="hover-theme-accent hover:bg-transparent cursor-pointer"
                      >
                        Logout
                      </a>
                    </li>
              ...
              <Link
                to="/reservation"
                className="btn theme-accent-bg hover:bg-white hover-theme-bg-text text-white border-none rounded-[4px] px-7 font-['Barlow_Condensed'] font-bold  tracking-wider hidden xl:inline-flex transition-all duration-300"
              >
                Reserve a Table
              </Link>
          ...
          <div className="absolute right-0 top-0 h-full w-[85%] max-w-sm theme-bg text-white shadow-2xl p-6 border-l border-white/10">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-3xl font-['Barlow_Condensed'] font-extrabold  italic tracking-wider">
                <span className="text-white">Khabar</span>
                <span className="theme-accent">Table</span>
              </h2>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="btn btn-circle btn-sm btn-ghost border border-white/20 hover-theme-accent-bg hover:text-white"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex flex-col gap-0 font-['Barlow_Condensed'] text-lg  tracking-wide">
              ...
              <Link
                to="/"
                onClick={() => setIsMobileMenuOpen(false)}
                className="border-b border-white/10 py-3 hover-theme-accent transition-colors"
              >
                Home
              </Link>
              <Link
              ...
              {user ? (
                <button
                  onClick={handleLogout}
                  className="text-left border-b border-white/10 py-3 hover-theme-accent transition-colors theme-accent"
                >
                  Logout
                </button>
              ) : (
                <Link
                  to="/login"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="border-b border-white/10 py-3 hover-theme-accent transition-colors theme-accent"
                >
                  Login
                </Link>
              )}
              ...
              <div className="mt-8">
                <Link
                  to="/reservation"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="btn theme-accent-bg hover:bg-white hover:text-black text-white border-none w-full font-bold  rounded-[4px]"
                >
                  Reserve Table
                </Link>
              </div>
