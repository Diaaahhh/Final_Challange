            ...
            {discPerc > 0 ? (
                <>
                    ...
                    <span className="text-base font-bold text-amber-600 whitespace-nowrap">
                        {effectivePrice.toLocaleString()} <span className="text-xs font-normal text-gray-500">Tk</span>
                    </span>
                </>
            ) : (
                <span className="text-base font-bold text-amber-600 whitespace-nowrap">
                    {basePrice.toLocaleString()} <span className="text-xs font-normal text-gray-500">Tk</span>
                </span>
            )}
          </div>
        </div>
          ...
          {isActive && (
            <div className="flex items-center gap-3 w-full">
              ...
              <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50 h-8">
                <button onClick={decrement} disabled={localQty === 0} className={`px-2 h-full rounded-l-lg transition-colors flex items-center justify-center ${localQty === 0 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-200'}`}>
                  <FaMinus size={8} />
                </button>
                <span className={`w-6 text-center font-bold text-sm ${localQty === 0 ? 'text-gray-400' : 'text-gray-800'}`}>
                    {localQty}
                </span>
                <button onClick={increment} className="px-2 h-full text-gray-600 hover:bg-gray-200 rounded-r-lg transition-colors flex items-center justify-center">
                  <FaPlus size={8} />
                </button>
              </div>

              {/* Add to Cart Button */}
              <button onClick={onAddToCart} disabled={isAddDisabled} className={`flex-1 h-8 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors flex items-center justify-center gap-2 shadow-sm ${isAddDisabled ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-[#C59D5F] hover:bg-[#0E1014] text-white cursor-pointer"}`}>
...
export default function MenuUser() {
  ...
  const [activeCategory, setActiveCategory] = useState({ id: "All", menu_name: "All Items" });
  ...
  const filteredItems = allMenuItems.filter(item => {
    ...
    const matchesBranch = String(item.m_branch_id) === String(selectedBranch) || branchArray.includes(String(selectedBranch));
    const matchesCategory = activeCategory.id === "All" || String(item.category_id) === String(activeCategory.id);
  ...
  const handleBranchSelect = (branchId) => {
    ...
    localStorage.setItem(CACHE_KEY, JSON.stringify({ branchId: branchId, timestamp: Date.now() }));
  };

  const currentBranchName = branches.find(b => String(b.branch_id) === String(selectedBranch))?.branch_name || "Unknown Branch";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          ...
          >
            ...
            <div className="bg-[#0E1014] p-6 text-center border-b border-gray-800 relative shrink-0">
              <button onClick={() => navigate(-1)} className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors" title="Go Back">
                <FaArrowLeft size={20} />
              </button>
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="truncate">
                          <h3 className="font-bold text-gray-800 group-hover:text-[#C59D5F] text-lg truncate">{branch.branch_name}</h3>
                          <p className="text-xs text-gray-500 truncate">{branch.name}</p>
                        </div>
                      </div>
                      <span className="text-gray-300 group-hover:text-[#C59D5F] text-xl ml-2 shrink-0">→</span>
                    </button>
                  ))}
                </div>
              ) : (
                <div className="text-center py-8 text-gray-500"><p>No branches available at the moment.</p></div>
              )}
            </div>
          </div>
        </div>
      ...
      <div className={`max-w-7xl mx-auto transition-opacity duration-500 ${showBranchModal ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
        ...
        <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-2">
             ...
             <span className="font-bold text-gray-800 uppercase">{currentBranchName}</span>
          </div>
          <button onClick={() => setShowBranchModal(true)} className="text-sm font-bold text-[#000000] hover:text-[#C59D5F] border-b-2 border-[#000000] hover:border-[#C59D5F] transition-all duration-300 uppercase tracking-wider pb-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-3 lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden sticky top-8 border border-gray-100">
              <div className="p-6 bg-gray-900 text-white"><h2 className="text-xl font-bold flex items-center gap-2 text-amber-400"><FaUtensils /> Categories</h2></div>
              <div className="flex flex-col max-h-[60vh] overflow-y-auto custom-scrollbar">
                
                <button onClick={() => setActiveCategory({ id: "All", menu_name: "All Items" })} className={`text-left px-6 py-4 transition-all duration-300 border-b border-gray-100 last:border-0 flex items-center justify-between group ${activeCategory.id === "All" ? "bg-amber-50 text-gray-900 border-l-4 border-l-amber-500 font-bold shadow-inner" : "text-gray-600 hover:bg-gray-50 hover:pl-8 border-l-4 border-l-transparent"}`}>
                  <span className="text-sm uppercase tracking-wider">All Items</span>
                </button>
                
                {allCategories.map((cat) => (
                  <button key={cat.id} onClick={() => setActiveCategory(cat)} className={`text-left px-6 py-4 transition-all duration-300 border-b border-gray-100 last:border-0 flex items-center justify-between group ${activeCategory.id === cat.id ? "bg-amber-50 text-gray-900 border-l-4 border-l-amber-500 font-bold shadow-inner" : "text-gray-600 hover:bg-gray-50 hover:pl-8 border-l-4 border-l-transparent"}`}>
          ...
          <div className="md:col-span-9 lg:col-span-9 flex flex-col gap-6">
            <div className="animate-fade-in-up">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6 flex justify-between items-center">
                ...
                <div className="px-3 py-1 bg-gray-100 rounded-full text-xs font-bold text-gray-500">{filteredItems.length} Items</div>
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                {loading ? (
                  <div className="col-span-2 text-center py-20 flex flex-col items-center"><span className="loading loading-spinner text-[#C59D5F] loading-lg"></span></div>
                ) : filteredItems.length > 0 ? (
                  filteredItems.map((item) => (
                    <MenuItemCard key={item.id} item={item} branchId={selectedBranch} branchName={currentBranchName} />
                  ))
                ) : (
                  <div className="col-span-2 p-10 text-center bg-white rounded-xl border border-gray-100">
                    ...
                    <p className="text-gray-400 font-bold tracking-wide">No items found in this category.</p>
                  </div>
                )}
              </div>
            </div>
